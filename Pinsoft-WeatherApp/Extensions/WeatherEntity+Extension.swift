...
extension ForecastEntity {
    func toForecast() -> Forecast {
        return Forecast(date: self.date ?? "", temperature: self.temperature, weatherDescription: self.weatherDescription ?? "", humidity: Int(self.humidity), windSpeed: self.windSpeed)
    }
}
